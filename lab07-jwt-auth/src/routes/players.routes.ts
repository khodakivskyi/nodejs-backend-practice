...
playersRouter.post("/", requireAuth, validate(createPlayerSchema), async(req, res) => {
    const player = await create(req.body, req.userId as string);
    res.status(201).json(player);
});

playersRouter.patch("/:id", requireAuth, validate(updatePlayerSchema), async(req, res) => {
    const updated = await update(req.params.id as string, req.body, req.userId as string);
    res.status(200).json(updated);
});

playersRouter.delete("/:id", requireAuth, async(req, res) => {
    await deleteEntity(req.params.id as string, req.userId as string);
    res.status(204).send();
});